...
const PAGE_SIZE = 30;
// Non-admin users must provide a date range; default to last month so the API always receives date_from/date_to.
const DEFAULT_DATE_PRESET: DateRangePresetOrAll = 'last_month';
...
export function ActivityLogsContent() {
  ...
  const [dateRange, setDateRange] = useState<DateRangeFilters>(
    getDateRangeForPreset(DEFAULT_DATE_PRESET)
  );
  ...
  const clearAllFilters = () => {
    ...
    setDateRange(getDateRangeForPreset(DEFAULT_DATE_PRESET));
    setOffset(0);
  };
  ...
  // Non-admin users must provide a date range (API returns 400 otherwise).
  const hasDateRangeForUser = Boolean(
    dateRangePreset !== 'all' && dateRange.dateFrom && dateRange.dateTo
  );
  const queryEnabled: boolean = isAdmin ? true : hasDateRangeForUser;
    ...
    <RequireAuth>
      <div className="container mx-auto px-4 py-6 max-w-7xl">
        ...
        <p className="text-gray-600 mb-4">
          {isAdmin
            ? 'User activity: login, logout, and create, save, delete actions. Shows user name for each entry.'
            : 'Your activity within the selected date range: login, logout, and create, save, delete actions.'}
        </p>
        {!isAdmin && (
          <p className="text-sm text-slate-600 mb-4 rounded-lg bg-slate-50 border border-slate-200 px-3 py-2">
            Select a date range to view your activity. Only activity within the chosen range is
            shown.
          </p>
        )}
          ...
          <div className="px-5 pb-5 pt-1 border-t border-slate-100">
            <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-4 mt-4">
              <div className="flex flex-col gap-1.5">
                ...
                >
                  {(isAdmin ? DATE_RANGE_PRESET_OPTIONS_WITH_ALL : DATE_RANGE_PRESET_OPTIONS).map(
                    (opt) => (
        ...
        {!queryEnabled && !isAdmin ? (
          <p className="text-gray-500">Select a date range above to view your activity.</p>
        ) : isLoading ? (
